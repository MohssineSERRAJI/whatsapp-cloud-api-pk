...
const createBot = (fromPhoneNumberId, accessToken, opts) => {
  let expressServer;
  const sendRequest = (0, sendRequestHelper_1.sendRequestHelper)(
    fromPhoneNumberId,
    accessToken,
    opts === null || opts === void 0 ? void 0 : opts.version
  );
  const getMediaPayload = (urlOrObjectId, options) => ({
    ...((0, isURL_1.default)(urlOrObjectId)
      ? { link: urlOrObjectId }
      : { id: urlOrObjectId }),
  ...
  return {
    startExpressServer: async (options) => {
      if (!expressServer) {
        expressServer = await (0, startExpressServer_1.startExpressServer)(
          options
        );
      }
